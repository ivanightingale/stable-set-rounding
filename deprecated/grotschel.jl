...
using JuMP, COSMO, SCS, MosekTools, COPT

# ╔═╡ 98b55ab3-c1ba-4c09-b9a2-0b942a022f8f
using Graphs

# ╔═╡ 25d63798-f6fe-4f58-8cd9-7913179f3eb8
using StatsBase

# ╔═╡ c7e640e3-148e-4967-b3b6-01d1dd4c5594
using Cairo, Fontconfig, GraphPlot, Compose, Colors

# ╔═╡ 2061d91c-421d-443e-9aae-f16f4e4525b0
begin
	using MatrixMarket
	use_complement = true
	graph_name = "san200-0-9-2"
	G = load_dimacs_graph(graph_name, use_complement)
	n = nv(G)
	i0 = n + 1
end

# ╔═╡ 7e8b2083-b509-440a-a79c-2573e5caa262
include("graph_utils.jl")

# ╔═╡ acb308e9-7afa-4361-9a1d-54c8a40a348b
include("params.jl");

# ╔═╡ 5bcb690e-5884-46f9-bad1-6454d0a692e8
include("opt_utils.jl");

# ╔═╡ d9adff19-2469-45bb-b66b-4be199ab9a1f
w = rand(n)
...
model = theta_sdp_model(solver="COSMO")
# model = theta_sdp_model(solver="SCS")

# ╔═╡ 1e370558-5bc7-4ede-a3ec-1dd377fbc927
@variable(model, X[1:n, 1:n]);

# ╔═╡ e06698e9-62f5-4086-9e1d-f5a810f9526d
@variable(model, x[1:n]);

# ╔═╡ a9b97122-e5ca-452b-9f55-ba0241c14b86
X_plus = vcat(hcat(X, x), hcat(x', 1));

# ╔═╡ e5419bb9-d200-47d1-a577-d241a6207334
@constraint(model, X_plus ∈ PSDCone());

# ╔═╡ 7eac4ba8-b0d0-4b89-a8ec-708f240d8f22
# @constraint(model, diag(X) .<= x);
@constraint(model, diag(X) .== x);

# ╔═╡ 18709c01-bd63-42bc-9723-d8a74db1470a
for e in edges(G)
    # @constraint(model, X[src(e), dst(e)] <= 0)
	@constraint(model, X[src(e), dst(e)] == 0)
end
...
# ╔═╡ b7b573dd-cf40-4018-b5bc-1cc1653bf51d
# include("fixed_point.jl");

# ╔═╡ da5f63c8-19dc-4875-86ad-2fdc7c197480
X_center = get_interior_point(model, X);

# ╔═╡ 26eee06c-c082-4c33-84a4-77bbe0d95d22
X_fixed_point = fixed_point_iteration(model, X, -X_center);
# X_fixed_point = fixed_point_iteration(model, X);
# X_fixed_point = fixed_point_iteration(model, X, -1/2 * ones((n,n)));

# ╔═╡ 8aa17f8d-fc60-46b5-ad0a-bb9d5f857b1f
eigvals(X_fixed_point)

# ╔═╡ 8bfb7807-0b02-4dc2-ae86-dc6f8c7965c1
greedy_stable_set_rounding(value.(X), G)
