...
# ╔═╡ 9e5be71f-e672-4d2d-8644-6aa16370d2d1
using Random

# ╔═╡ 211fbd13-b176-4164-bbf9-78be18016ca7
using MatrixMarket

# ╔═╡ 4421cdd3-5be0-4c78-919d-02550bf70d2c
using StatsBase

# ╔═╡ d40ac63e-9c00-4316-95b9-27c46a1f56d1
include("params.jl");

# ╔═╡ 01686123-a123-4eeb-8ba4-b0e980916093
include("opt_utils.jl");

# ╔═╡ 9f841a49-6635-4db2-9820-df6d21e638a6
include("fixed_point.jl");

# ╔═╡ d512923d-1fef-4666-82d8-35918320c20d
G, n, i0 = load_graph();
...
@variable(model, X[1:i0, 1:i0]);

# ╔═╡ e5419bb9-d200-47d1-a577-d241a6207334
@constraint(model, X ∈ PSDCone());

# ╔═╡ 7eac4ba8-b0d0-4b89-a8ec-708f240d8f22
@constraint(model, diag(X) .== ones(i0));

# ╔═╡ 0bce4876-eca8-409f-89b0-6a0de39b9127
for e in edges(G)
	i = src(e)
	j = dst(e)
    @constraint(model, X[i,i] + X[j,j] + X[i0,i0] + 2X[i,j] + 2X[i,i0] + 2X[j,i0] == 1);
end

# ╔═╡ 6bc5fa9e-fabd-4de2-a4b0-baba8ede2187
M = Matrix(Symmetric(sparse(collect(1:n), collect(1:n), 1/2, i0, i0) + sparse(collect(1:n), fill(i0, n), 1/4, i0, i0)));

# ╔═╡ 948cdb7c-6e91-42aa-8d55-08b18fe7a62e
@objective(model, Max, tr(M * X));

# ╔═╡ dfa6682c-a060-46c4-bf87-0247053bc8bf
optimize!(model);

# ╔═╡ 3755a5b9-4a7d-4d26-95db-766ec5cbd770
solution_summary(model)

# ╔═╡ 777c7ca7-aa47-45af-bd95-e93a637d4675
X_center = get_interior_point(model, X);

# ╔═╡ 6e860a19-c299-4083-abff-2bde7e0c9a45
# X_fixed_point = fixed_point_iteration(model, X, -X_center);
X_fixed_point = fixed_point_iteration(model, X);

# ╔═╡ f7d73ff9-9260-47f0-acd5-42f8c226a98b
eigvals(X_fixed_point)

# ╔═╡ 168b4830-0439-449c-8157-836c2ebc31c1
hyperplane_rounding(value.(X), G)
