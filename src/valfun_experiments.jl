...
# Obtain a value function, and run tabu_valfun_test().
function run_tabu_valfun_test(G, w, params, use_qstab=false)
    out = get_params_valfun(G, w, params, use_qstab)
    obj = out.sol.value
    println("Optimal value: ", obj)
    val = out.val
    tabu_valfun_test(G, w, obj, val;
        ϵ=params[:valfun_ϵ],
        solver=params[:solver],
        solver_ϵ=params[:solver_ϵ],
        feas_ϵ=params[:solver_feas_ϵ],
        verbose=params[:verbose]
    )
end

# Obtain a value function, and run round_valfun().
...
function run_tabu_valfun_compare(G, w, sdp_params, qstab_params)
    ...
    tabu_valfun_compare(G, w, θ, val_sdp, val_qstab;
        ϵ=sdp_params[:valfun_ϵ],
        verbose=sdp_params[:verbose]
    )
end

# Find all dual BFS of a QSTAB LP, and test the value function generated by each of them.
# Then, take the average of the BFS to obtain an interior point, and test its corresponding
# value function.
...
# Find a combination of "bad" value function and weights, and check if the SDP value function
# still works for this set of bad weights (by running tabu_valfun_test()).
function run_find_bad_valfun(G, params)
    ...
    println("Bad weights: ", bad_w)
    
    # Check whether bad_val is indeed bad (whether there exists a vertex that is not in
    # any maximum stable set)
    ...
    tabu_valfun_test(G, bad_w, obj, sdp_out.val;
        ϵ=params[:valfun_ϵ],
        solver=params[:solver],
        solver_ϵ=params[:solver_ϵ],
        feas_ϵ=params[:solver_feas_ϵ],
